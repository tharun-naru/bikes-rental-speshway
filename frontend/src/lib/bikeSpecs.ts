...
/**
 * Returns the correct brand for a given model name.
 * If model is not found in predefined specs, returns null.
 */
export const getBrandForModel = (modelName: string): string | null => {
...
/**
 * Validates if a brand-model pair is correct according to predefined specs.
 * If model is unknown, it's considered valid for any brand (for custom additions).
 */
export const validateBrandModelMatch = (brand: string, model: string): boolean => {
  const correctBrand = getBrandForModel(model);
  if (!correctBrand) return true; // Allow unknown models
